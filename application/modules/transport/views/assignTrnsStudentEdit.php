            ...
            <div class="col-md-12 ">
                ...
                <div class="portlet box green ">
                    ...
                    <div class="portlet-body form">
                        ...
                        <?php      
                        // echo $info[0]['student_id'];                  
                        if (!empty($success)) {
                              echo $success;
                        }
                        $form_attributs = array('class' => 'form-horizontal', 'role' => 'form');
                        echo form_open_multipart('transport/assignTrnsStudentEdit/'.$info[0]['id'], $form_attributs);
                        ?>
                        <div class="form-body">
                                    ...
                                    <div class="col-md-6">
                                        <select name="class_id" id="class" onchange="classInfo(this.value)" class="form-control" data-validation="required" data-validation-error-msg="<?php echo lang('admi_Class_error_msg'); ?>">
                                            ...
                                            <?php foreach ($s_class as $row) { ?>
                                                <option value="<?php echo $row['id'];?>" <?php echo $info[0]['class_id']==$row['id']?'selected':''; ?> ><?php echo $row['class_title']; ?></option>
                                            <?php } ?>
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="col-md-5 col-md-offset-3">
                                <div class="form-group">
                                    ...
                                    <select name="studentId" onchange="studentInfo(this.value)" id="stdId" class="form-control" data-validation="required">
                                        ...
                                        {
                                        $selected = $info[0]['student_id']==$studentId['student_id']?'selected':'';
                                            echo '<option value="' . $studentId['student_id'].'" '.$selected.'>' . $studentId['student_id'].'  ( '.$studentId['roll_number'].' - '.$studentId['student_nam']. ' )</option>';
                                        }
                                        ?>
                                        </select> 
                                    </div>
                                    ...
                                    <div class="form-group">
                                    <label class="control-label">Select Route<span class="requiredStar"> * </span></label>

                                    <select name="route_id" class="form-control" data-validation="required">
                                        <option value="">Select one</option>
                                        <?php 
                                        foreach ($trans_route as $row) 
                                        {
                                            $selected = $info[0]['route_id']==$row['id']?'selected':'';
                                            echo '<option value="' . $row['id'] .'" '.$selected.'>' . $row['rout_title'].'</option>';
                                        }
                                        ?>
                                        </select> 
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="input-group date" data-provide="datepicker">
                                            <input type="text" name="date" class="form-control" value="<?php echo date("d-m-Y", strtotime($info[0]['enroll_date']))?>" data-validation="required" >
