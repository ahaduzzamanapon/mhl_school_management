...
class Subjects extends MX_Controller {
    ...
    public function classSubjectEdit() {
        ...
        if($this->input->post('submit', TRUE)) {
           ...
           $now_id = $this->db->escape_like_str($this->input->post('id', TRUE));

           $count_id = count($now_id);
           for($i=0;$i<=$count_id;$i++) {
           $sub_title = 'subject_title'.'_'.$now_id[$i];
           $writer_name = 'writer_name'.'_'.$now_id[$i];
           $edition = 'edition'.'_'.$now_id[$i];

          $subj = $this->db->escape_like_str($this->input->post($sub_title, TRUE));
          $writer = $this->db->escape_like_str($this->input->post($writer_name, TRUE));
          $edition = $this->db->escape_like_str($this->input->post($edition, TRUE));

          $this->db->where('id',$now_id[$i]);
          $this->db->update('class_subject', ['subject_title'=>$subj, 'writer_name'=>$writer, 'edition'=>$edition]);
        }
       /* echo "<pre>";
        print_r($subject_data);*/
        //$this->db->where('class_id', $id);
        //$this->db->where_in('id', $now_id);
        //$this->db->update('class_subject', $subject_data);

        redirect('subjects/allSubject');
      }
