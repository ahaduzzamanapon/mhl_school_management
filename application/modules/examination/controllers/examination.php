...
class Examination extends MX_Controller {
    ...
    public function completExamRoutin() {
        if ($this->input->post('submit', TRUE)) {
            ...
            if ($this->input->post('examSunjectFild', TRUE)) {
                ...
                $examDay = $this->input->post('day', TRUE);
                ...
                $routine = array(
                    ...
                    'exam_day' => $this->db->escape_like_str($examDay),
    ...
    public function editCompletExamRoutin() {
        if ($this->input->post('submit', TRUE)) {
            //$examId = $this->input->post('examId', TRUE);
        ...
        else
        {
            ...
            //$data['rutineInfo'] = $this->common->getWhere('exam_routine', 'exam_id', $examId);
            $data['rutineInfo'] = $this->common->getWhere('exam_routine', 'id', $rid);
            /*echo "<pre>";
            print_r($data);exit;*/
            $data['examInfo'] = $this->common->getWhere('add_exam', 'id', $examId);
            $data['schoolName'] = $this->common->schoolName();
            ...
            $this->load->view('temp/header');
            $this->load->view('editRutineSubject', $data);
            $this->load->view('temp/footer');
        }
    }
        ...
        if ($this->input->post('submit', TRUE)) {
            ...
            if ($check == 'Have An Exam') {
                ...
                if (!empty($data['students'])) {
                    ...
                    echo $data['classTitle']  . 'has no any student.';
                }
    ...
    //Here is first time select class for result.subjectTitle
    ...
    public function submitResult() {
        ...
        for ($a = 1; $a <= $i; $a++) {
            ...
            $g_Info = $this->common->getWhere_grade($mark);

            $point=$g_Info[0]['point'];
            $grade=$g_Info[0]['grade_name'];
            if ($point>0) {$result= 'Pass';}
            else{$result= 'Fail';}
            $resultInfo = array(
                'exam_id' => $this->db->escape_like_str($examID),
                'exam_title' => $this->db->escape_like_str($examTitle),
