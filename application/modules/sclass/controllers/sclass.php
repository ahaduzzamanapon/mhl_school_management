...
class Sclass extends MX_Controller
{
    ...
    public function selectAllRoutine()
    {
        ...
        //dd($user);
        $data = [];
        $teacherInfo = $this->db->where('user_id', $user->id)->get('teachers_info')->row();
        //dd($teacherInfo);
        if ($teacherInfo) {
            $classInfo = $this->db->where('subject_teacher', $user->username)->get('class_routine')->result();
            //dd($classInfo);
            $data['classTile'] = [];

            foreach ($classInfo as $row) {
                $classTile = $this->db->where('id', $row->class_id)->get('class')->row();
                if ($classTile) {
                    $data['classTile'][] = get_object_vars($classTile);
                }
            }
        } else {
            $data['classTile'] = $this->common->getAllData('class');
